...
//   const [hovered, setHovered] = useState(false)
  ...
  return (
    ...
    //   onMouseLeave={() => setHovered(false)}
      ...
      className={cn(
        "relative w-60 h-60 rounded-3xl border border-border/20 text-card-foreground overflow-hidden shadow-xl shadow-amber-400/30 cursor-pointer group backdrop-blur-sm",
    ...
    >
      ...
      {/* Content */}
      <motion.div 
        variants={contentVariants as import("framer-motion").Variants}
        initial="hidden"
        animate="visible"
        className="absolute bottom-0 left-0 right-0 p-6 space-y-4"
      >
        {/* Name and Verification */}
        <motion.div variants={itemVariants as import("framer-motion").Variants} className="flex items-center gap-2">
          <motion.h2 
          ...
          >
            {name.split("").map((letter, index) => (
              <motion.span
                key={index}
                variants={letterVariants as import("framer-motion").Variants}
